...
import com.aca.pimp.CheckVehicleWS.CheckVehicle;
                        ...
                        try {
                            if (pro_name.toUpperCase().equals("ASRI")) {
                                ...
                                if (!Scalar.isNewSppa(SyncUnsyncActivity.this, i)) {
                                    Toast.makeText(SyncUnsyncActivity.this, "Proses sinkronisasi tidak bisa diproses untuk sppa otomate yang dibuat pada versi aplikasi sebelumnya", Toast.LENGTH_SHORT)
                                            .show();
                                    return;
                                }
    ...
    private boolean needSync() {
        try {
            ...
            if (cMain.moveToFirst()) {
                ...
                LocalDate mEntryDate = com.aca.util.UtilDate.toDate(entryDate, com.aca.util.UtilDate.STANDARD_DATE);
                dbMain.close();
                if (versionAndroid.Version >= 36 && mEntryDate.isBefore(com.aca.util.UtilDate.toDate(versionAndroid.DateTime))) {
                    return true;
                }
            }
            dbMain.close();
