...
//		startScrollingThread();
	...
	private void getIntentData() {
		...
		if (getIntent().getExtras() != null) {
			...
			LoadPimpTravel ws = new LoadPimpTravel(this, hmap) {

				@Override
				public void getTravelList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
...
//							namaGrupTour.setText(hMap.get("CUSTOMER_NAME"));
//							jumlahTertanggung.setText(hMap.get("CUSTOMER_NAME"));
//						
							
							for (int i = 0; i < spTujuan.getCount(); i++) {               
								String item = spTujuan.getItemAtPosition(i).toString(); 
					            if (item.equalsIgnoreCase(hMap.get("TRAVEL_NOTE"))) {
							...
							if (Integer.parseInt(hMap.get("PRODUCT_PLAN_2_FLAG")) == 1) 
								fam.setChecked(true);
							else if (Integer.parseInt(hMap.get("PRODUCT_PLAN_2_FLAG")) == 0)
								individu.setChecked(true);
							else 
								grup.setChecked(true);
							
							if (Integer.parseInt(hMap.get("TRAVEL_COUNTRY_FLAG")) == 1) 
...
//							schengne
							...
							namanegara1.setText(hMap.get("TRAVEL_COUNTRY"));
							namanegara2.setText(hMap.get("TRAVEL_COUNTRY_NAME2"));
							namanegara3.setText(hMap.get("TRAVEL_COUNTRY_NAME3"));
							namanegara4.setText(hMap.get("TRAVEL_COUNTRY_NAME4"));
							namanegara5.setText(hMap.get("TRAVEL_COUNTRY_NAME5"));
							namanegara6.setText(hMap.get("TRAVEL_COUNTRY_NAME6"));
					 
								
							namaahliwaris.setText(hMap.get("BENEFICIARY_NAME"));
							hub.setText(hMap.get("BENEFICIARY_RELATION"));
							
							etFromDate.setText(hMap.get("DEPARTURE_DATE"));
							etToDate.setText(hMap.get("ARRIVAL_DATE"));
							
							if(Integer.parseInt(hMap.get("PRODUCT_PLAN_1_FLAG")) == 0)
								vip.setChecked(true);
							else if(Integer.parseInt(hMap.get("PRODUCT_PLAN_1_FLAG"))== 1)
								executive.setChecked(true);
							else if(Integer.parseInt(hMap.get("PRODUCT_PLAN_1_FLAG")) == 2)
								duluxe.setChecked(true);
							else if(Integer.parseInt(hMap.get("PRODUCT_PLAN_1_FLAG"))== 3)
								superior.setChecked(true);
							
							if(Integer.parseInt(hMap.get("DCOD")) == 0)
								annual.setChecked(true); 
							else 
								annual.setChecked(false);
							
							jumlahhari.setText(nf.format(Double.parseDouble(hMap.get("PREMI_DAYS"))));
							tambahan.setText(nf.format(Double.parseDouble(hMap.get("PREMI_WEEKS"))));
							
							Double loadingPremi = Double.parseDouble(hMap.get("TOTAL_PREMIUM")) - Double.parseDouble(hMap.get("PREMI_DAYS"))- Double.parseDouble(hMap.get("PREMI_WEEKS")); 
							loadingpremi.setText(nf.format(loadingPremi));							 
							nopass.setText(hMap.get("TRAVEL_PASSPORT_NO"));
			...
			LoadPimpFamily wsfam = new LoadPimpFamily(this, hmap) {
				
				@Override
				protected void getFamilyList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
						
						fillFam(arrList.get(1), namapasangan, etPasangan, nopasspasangan, pasangan);
						fillFam(arrList.get(2), namaanak1, etAnak1, nopassanak1, anak1);
						fillFam(arrList.get(3), namaanak2, etAnak2, nopassanak2, anak2);
						fillFam(arrList.get(4), namaanak3, etAnak3, nopassanak3, anak3); 
					}						
				}

				private void fillFam(	HashMap<String, String> hMap,
										EditText nama,
										EditText dob,
										EditText ktp,
										Switch swi) 
				{
					nama.setText(hMap.get("NAME"));
					dob.setText(hMap.get("DOB"));
					ktp.setText(hMap.get("ID_NO"));
					
					if(!hMap.get("NAME").isEmpty()) swi.setChecked(true); 
				}
			};
	...
	private LinearLayout layApprove;
...
public void btnDeleteClick(View v){
	...
	public void btnNextClick(View v) {
		if (PRODUCT_ACTION.equals("APPROVE")) {
			...
			b.putDouble("TSI", 0.0);
			i.putExtras(b);
			Utility.setTempactivity((Activity)FillTravelActivity.this);
			startActivity(i);
		}
		else { 
...
//		}
	...
	private void getPremi(){
		...
		String Area, Coverage, CCOD = "", DCOD = null, TotalDays, TotalWeeks;
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillTravelActivity.this, "Informasi", "Produksi gagal di-approve").show();
			}
		};
		ws.execute(); 
	}
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillTravelActivity.this, "Informasi", "Produksi gagal di-reject").show();
