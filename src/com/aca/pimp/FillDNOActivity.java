...
//			MAX_DISCOUNT = Utility.getMaxDiscount(getBaseContext(), "29");
	...
	private void getIntentData() {
		...
		if (getIntent().getExtras() != null) {
			...
			LoadPimpDNO ws = new LoadPimpDNO(this, hmap) {

				@Override
				public void getDNOList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
						try {
							...
							if(hMap.get("Q1_FLAG").equals("1"))
								q1flag.setChecked(true);	
							
							q1note.setText(hMap.get("Q1_NOTE"));						
							etQ1Date.setText(hMap.get("Q1_DATE"));
							...
							if(hMap.get("PRODUCT_PLAN_FLAG").equals("1")){
								plan1.setChecked(true);
							}else if(hMap.get("PRODUCT_PLAN_FLAG").equals("2")){
			...
			LoadPimpCompany wsCompany = new LoadPimpCompany(this, hmap) {
				
				@Override
				protected void getCompanyList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) { 
						
						setCompanyWidget(arrList.get(0), spCompBusType1,compname1);
						setCompanyWidget(arrList.get(1), spCompBusType2,compname2);
						setCompanyWidget(arrList.get(2), spCompBusType3,compname3);
						setCompanyWidget(arrList.get(3), spCompBusType4,compname4);
						setCompanyWidget(arrList.get(4), spCompBusType5,compname5);
						setCompanyWidget(arrList.get(5), spCompBusType6,compname6);
						setCompanyWidget(arrList.get(6), spCompBusType7,compname7);
						setCompanyWidget(arrList.get(7), spCompBusType8,compname8);
						setCompanyWidget(arrList.get(8), spCompBusType9,compname9);
						setCompanyWidget(arrList.get(9), spCompBusType10,compname10);
				...
				private void setCompanyWidget(HashMap<String, String> hMap, Spinner spn, EditText txt) {
					SpinnerGenericAdapter a = (SpinnerGenericAdapter)spCompBusType1.getAdapter();
					spn.setSelection(a.getItemId(hMap.get("COMPANY_BUSINESS_TYPE")));
					txt.setText(hMap.get("COMPANY_NAME")); 
				}
			};
			wsCompany.execute();
		}
	...
	public void btnNextClick(View v) {
		if (PRODUCT_ACTION.equals("APPROVE")) {
			...
			b.putDouble("TSI", 0.0);
			i.putExtras(b); 
			Utility.setTempactivity((Activity)FillDNOActivity.this);
			startActivity(i);
		}
		else { 
			try{
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillDNOActivity.this, "Informasi", "Produksi gagal di-approve").show();
			}
		};
		ws.execute(); 
	}
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillDNOActivity.this, "Informasi", "Produksi gagal di-reject").show();
