...
import com.aca.pimp.CheckCustomerIdWS.CallBackListener;
...
public class CustomerActivity extends ControlNormalActivity implements CallBackListener, InterfaceCustomer {
    ...
    private PropertyInfo pJenisID = new PropertyInfo();
	...
	private Spinner phoneCodeSpn, jenisIdSpn;
	
	private Button btnPrev, btnNext, btnCheckCustomer;
	private View sectionMiddle, sectionButton; 
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(cust_action.equals("EDIT")){
			LoadCustomer();
			((com.aca.pimp.CustomTextViewBold)findViewById(R.id.lblTitle)).setText("Edit Nasabah");
...
//				LOAD CUSTOMER
	...
	private class InsertCustomerInfoWS extends AsyncTask<String, Void, Void>{
		...
		private SoapObject responseBody;
		...
		protected Void doInBackground(String... params) {
			...
			try{
		        ...
		        pJenisID.setName("JenisID");         
		        pJenisID.setValue(((SpinnerGenericItem)jenisIdSpn.getSelectedItem()).getCode());         
		        pJenisID.setType(String.class);         
		        requestinsert.addProperty(pJenisID);
...
//	    		Log.i(TAG, "::doInBackground:" + "" + response);
	            ...
	            response  = responseBody.getPropertySafelyAsString("InsertCustomerResult");	   
	            Log.i(TAG, "::doInBackground:" + "" + response);
		...
		protected void onPostExecute(Void result) {
			...
			try{
				...
				if(error){
					...
					Toast.makeText(CustomerActivity.this, "Customer baru berhasil dibuat dengan nomor ID " + response, Toast.LENGTH_SHORT).show();
				}
	...
	private class UpdateCustomerInfoWS extends AsyncTask<String, Void, Void>{
		...
		protected Void doInBackground(String... params) {
			...
			try{
		        ...
		        pJenisID.setValue(((SpinnerGenericItem)jenisIdSpn.getSelectedItem()).getCode());         
		        pJenisID.setType(String.class);         
		        requestupdate.addProperty(pJenisID);
...
//	    		SoapPrimitive response = (SoapPrimitive)envelope.getResponse();
	    		responseBody = (SoapObject) envelope.bodyIn;
	            response  = responseBody.getPropertySafelyAsString("UpdateCustomerResult");	            
	            Log.d("customer update", response.toString());
		...
		protected Void doInBackground(String... params) {
			...
			try{
	            ...
	            for(int i = 0; i < iTotalDataFromWebService; i++)
	            {
		            ...
		            map.put("JENIS_ID", tableRow.getPropertySafelyAsString("JENIS_ID").toString());
	...
	public void fillCustomer(HashMap<String, String> map) {
		cust_code = map.get("CUSTOMER_NO").toString();
...
//		dengan 62 -> 62626252
		...
		a =  (SpinnerGenericAdapter)jenisIdSpn.getAdapter();
		jenisIdSpn.setSelection(a.getItemId(map.get("JENIS_ID").toString()));
		...
		a = (SpinnerGenericAdapter)spOccupation.getAdapter();
