...
//		data.run();
	...
	private void disableView() {
		...
		String statusLevel = map.get("StatusLevel");
		
		if(roleid != 1 || statusLevel.equalsIgnoreCase("W"))  {
			findViewById(R.id.colCustomer).setVisibility(View.GONE);
			findViewById(R.id.colProduct).setVisibility(View.GONE);
			findViewById(R.id.colSync).setVisibility(View.GONE);
			findViewById(R.id.colSPPA).setVisibility(View.GONE); 
...
//		   toast("Alarm Started...");
  ...
  protected void onResume() {
      ...
      IntentFilter filter = new IntentFilter("com.aca.pimp.TaskService");
...
//	            	cancelAlarm();
	...
	public void btnProductReportClick(View v)
	{
		if(!Utility.cekLogin(getBaseContext())) {
			Intent i = new Intent (getBaseContext(), RegisterOrLoginActivity.class);
			Bundle b = new Bundle ();
		...
		{
			Intent i = new Intent(getBaseContext(),  ChooseProductionMenuActivity.class);
			startActivity(i);
			this.finish();
		}
	}
