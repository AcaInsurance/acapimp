...
public class FillKonvensionalActivity extends ControlNormalActivity  {
	...
	private LinearLayout layLoading5tahun, layNext, layApprove;
	...
	private void getKonvensionalType() {		
		if (PRODUCT_ACTION.equals("APPROVE")) 
			return;
...
//		}
	...
	private void init() {
		try {
			...
			else if (PRODUCT_ACTION.equals("APPROVE")) {
				disableListener();
				getIntentData();
				disableView();
//				changeLayout();
...
//			MAX_DISCOUNT = Utility.getMaxDiscount(getBaseContext(), "21");
	...
	private void changeLayout() {
		...
		default:
			if (b.getString("APPROVAL").equalsIgnoreCase("1")) {
				findViewById(R.id.btnNext).setVisibility(View.GONE);	
				findViewById(R.id.btnDelete).setVisibility(View.GONE);
			}
			else {
				layApprove.setVisibility(View.VISIBLE);
	...
	private void getIntentData() {
		...
		if (getIntent().getExtras() != null) {
			SPPAno = getIntent().getExtras().getString("SPPA_NO");
			hmap.put("SPPA_NO", SPPAno);
			
			LoadPimpOtomate ws = new LoadPimpOtomate(this, hmap) {

				@Override
				public void getOtomateList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
//						{VEHICLE_CATEGORY=2, VEHICLE_MERK_DESC=BIMANTARA, PENGGUNAAN=D, TIPE_RATE=, SEATING_CAP=4, CUSTOMER_NAME=MARSEL, NON_STANDARD_ACCESORIES=ASDAS, TS=, ADD_SI=0.00, EXP_DATE=2016-09-10T00:00:00+07:00, VEHICLECATG=11, VEHICLECATG_DESC=SEDAN/JEEP, CUSTOMER_NO=0015000001, ENGINE_NO=ASD, TPL=40000000.00, VEHICLE_MERK=11, TIPE_LOADING=2, DAMAGE_REMARK=ASDASD, PLAT_NO_1=B, RATE=4.20000, BUILT_YEAR=2008, PLAT_NO_3=22, SI=120000000.00, PLAT_NO_2=1233, TOTAL_PREMIUM=5550000.00, FLOOD=, EFF_DATE=2015-09-10T00:00:00+07:00, CHASSIS_NO=ASD, VEHICLE_CATEGORY_DESC=CAKRA 1.5, SRCC=, ACCESORIES_FLAG=0, COLOUR=ASD, EQ=, DAMAGE_FLAG=1, OWN_RISK=500000, VEHICLE_DESC=ASDAS, PA=50000000.00, COVERAGE_FLAG=0}
						
						try {
							HashMap<String, String> hMap = arrList.get(0);
							customerno.setText(hMap.get("CUSTOMER_NO"));
							customername.setText(hMap.get("CUSTOMER_NAME"));
				
							carIDBrand = hMap.get("");
							carIDType = hMap.get("");
				
							merkKendaraan.setTag(hMap.get("VEHICLE_MERK"));
							merkKendaraan.setText(hMap.get("VEHICLE_MERK_DESC"));
							tipeKendaraan.setTag(hMap.get("VEHICLE_CATEGORY"));
							tipeKendaraan.setText(hMap.get("VEHICLE_CATEGORY_DESC"));
				
				
							model.setText(hMap.get("VEHICLE_DESC"));
				 
							if (hMap.get("COVERAGE_FLAG").equalsIgnoreCase("0")) {
							...
							tahun.setText(hMap.get("BUILT_YEAR"));
							nopol1.setText(hMap.get("PLAT_NO_1"));
							nopol2.setText(hMap.get("PLAT_NO_2"));
							nopol3.setText(hMap.get("PLAT_NO_3"));
				
							color.setText(hMap.get("COLOUR"));
							chassisNo.setText(hMap.get("CHASSIS_NO"));
							machineNo.setText(hMap.get("ENGINE_NO"));
							perlengkapan.setText(hMap.get("NON_STANDARD_ACCESORIES"));
				
							seat.setText(String.valueOf(hMap.get("SEATING_CAP")));
							jangkaWaktuEff.setText(hMap.get("EFF_DATE"));
							jangkaWaktuExp.setText(hMap.get("EXP_DATE"));
							nilaiPertanggungan.setText(nf.format(Double.parseDouble(hMap.get("SI"))));
							nilaiPerlengkapan.setText(nf.format(Double.parseDouble(hMap.get("ADD_SI"))));
				

							if (hMap.get("FLOOD").equals("1"))
								AOG.setChecked(true);
							else
								AOG.setChecked(false);
							 
							if (hMap.get("EQ").equals("1"))
								EQ.setChecked(true);
							else
								EQ.setChecked(false);
							 
							if (hMap.get("SRCC").equals("1"))
								SRCC.setChecked(true);
							else
								SRCC.setChecked(false);
							
							if (hMap.get("TS").equals("1"))
								TS.setChecked(true);
							else
								TS.setChecked(false);
							  
							
							if (hMap.get("TIPE_RATE").equals("3"))
								rbRateAtas.setChecked(true);
							else if (hMap.get("TIPE_RATE").equals("2"))
								rbRateTengah.setChecked(true);
							else if (hMap.get("TIPE_RATE").equals("1"))
							...
							spnTpl.setSelection(a.getItemId(Double.parseDouble(hMap.get("TPL"))));

							a =  (SpinnerGenericAdapter)spnPA.getAdapter();
							spnPA.setSelection(a.getItemId(Double.parseDouble(hMap.get("PA"))));
				
							a =  (SpinnerGenericAdapter)spnDNC.getAdapter();
							spnDNC.setSelection(a.getItemId(hMap.get("PENGGUNAAN")));
							
							
							if (hMap.get("DAMAGE_FLAG").equals("1"))
								swiKerusakan.setChecked(true);
							else 
								swiKerusakan.setChecked(false);
							
							keteranganKerusakan.setText(hMap.get("DAMAGE_REMARK"));
							
							if (!hMap.get("TIPE_LOADING").equals("2"))
								rbLoadingRate.setChecked(true);
							else  
								rbLoadingResikoSendiri.setChecked(true);

							risikoSendiri.setText(nf.format(Double.parseDouble(hMap.get("OWN_RISK"))));
							
							rate.setText(nf.format(Double.parseDouble(hMap.get("RATE"))));
							premi.setText(nf.format(Double.parseDouble(hMap.get("TOTAL_PREMIUM"))));
				//			discpct.setText(nf.format(hMap.get(""));
				//			disc.setText(nf.format(hMap.get(""));
				//			polis.setText(nf.format(hMap.get(""));
				//			materai.setText(nf.format(hMap.get(""));
							total.setText(nf.format(Double.parseDouble(hMap.get("TOTAL_PREMIUM"))));
				
				
							if (hMap.get("ACCESORIES_FLAG").equals("1"))
	...
	private void disableView() {
		...
		rbLoadingRate.setEnabled(false);
		rbLoadingResikoSendiri.setEnabled(false);
...
//						validasiNilaiTSI();
		...
		tahun.addTextChangedListener(new TextWatcher() {
			...
			public void afterTextChanged(Editable s) {
				// TODO Auto-generated method stub
				Log.i(TAG, "::afterTextChanged:" + "tahun listener masuk");
				tahun.setTextColor(Color.BLACK);
				try {
					if (!s.toString().equals(current)) {
	...
	private void InitEditText() {
		...
		layNext			 = (LinearLayout) findViewById(R.id.layNext);
		layApprove		 = (LinearLayout) findViewById(R.id.layApprove);
...
//				maxTahun = 7;
	...
	public void btnHomeClick(View v) {
		...
		if (PRODUCT_ACTION.equalsIgnoreCase("APPROVE")) {
	...
	public void btnNextClick(View v) throws ParseException {
		if (PRODUCT_ACTION.equals("APPROVE")) {
			Intent i = new Intent(getBaseContext(),  PhotoShowActivity.class);
			b.putString("PRODUCT_TYPE", "KONVENSIONAL");
			b.putDouble("TSI", getTSI());
			i.putExtras(b);
			Utility.setTempactivity((Activity)FillKonvensionalActivity.this);
			startActivity(i);
		}
		else { 
			try{
				...
				validasiNext();
				if(flag != 0)
					Utility.showCustomDialogInformation(getBaseContext(), "Warning", UIErrorMessage);
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillKonvensionalActivity.this, "Informasi", "Produksi gagal di-approve").show();
			}
		};
		ws.execute(); 
	}
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillKonvensionalActivity.this, "Informasi", "Produksi gagal di-reject").show();
