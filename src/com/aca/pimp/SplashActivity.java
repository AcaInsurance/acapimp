...
public class SplashActivity extends Activity implements RetriveLastVersionAppsListener {
    ...
    private Boolean bNeedSyncMarineRate = statusSync;
    ...
    private Boolean bFinishMarineRate = statusFinished;
    ...
    private GetRateMarine wsMarineRate;
...
//					SplashActivity.this.finish();
    ...
    protected void EmptySharedPreferences() {
        ...
        SharedPreferences sharedPreferences = SplashActivity.this.getSharedPreferences("com.aca.pimp", Context.MODE_PRIVATE);
        sharedPreferences
                .edit()
...
//			Utility.CopyDB( getBaseContext().getAssets().open("AMM_VERSION_BIG"), new FileOutputStream(destPath));
    ...
    private void SyncData() {
        ...
        try {
            if (error) {
                ...
                Log.d("Sync. Table Version", "Success");
                ...
                if (bNeedSyncMarineRate) {
                    wsMarineRate = new GetRateMarine(SplashActivity.this, new HashMap<String, String>()) {

                        @Override
                        protected void getRateMarine(ArrayList<HashMap<String, String>> arrList) {
                            InsertRateMarine(arrList);
                        }
                    };
                    wsMarineRate.execute();
                } else
                    bFinishMarineRate = true;
    ...
    public void SetNeedSyncFlag(String s) {
        ...
        else if (s.equals("RateMarine"))
            bNeedSyncMarineRate = true;
        ...
        bNeedSyncACAMobilRate = false;
    }

    private void InsertRateMarine(ArrayList<HashMap<String, String>> arrList) {
        DBA_TABLE_VERSION dbVersion = new DBA_TABLE_VERSION(SplashActivity.this);

        if (arrList == null) {
            bFinishMarineRate = false;

            dbVersion.open();
            dbVersion.setNeedSync("RateMarine");
            dbVersion.close();

            Log.d("Sync. Marine Rate ", "Fail");
            errorSync = true;
        } else {
            DBA_MASTER_MARINE_RATE dbMarineRate = new DBA_MASTER_MARINE_RATE(this);

            try {
                dbMarineRate.open();
                dbMarineRate.deleteAll();
                for (HashMap<String, String> hashMap : arrList) {
                    dbMarineRate.insert(
                            Double.parseDouble(hashMap.get("RATE_BAWAH"))
                            , Double.parseDouble(hashMap.get("RATE_TENGAH"))
                            , Double.parseDouble(hashMap.get("RATE_ATAS"))
                    );
                }

                dbVersion.open();
                dbVersion.setSuccessSync("RateMarine");
                dbVersion.close();

                bFinishMarineRate = true;
                Log.d("Sync. Marine Rate", "Success");

            } catch (Exception e) {
                e.printStackTrace();
            } finally {
                if (dbMarineRate != null)
                    dbMarineRate.close();
            }
        }
    }
        ...
        protected Void doInBackground(String... params) {
            ...
            try {
                ...
                for (int i = 0; i < iTotalDataFromWebService; i++) {
                    ...
                    db.insert(Utility.isAnytype(tableRow.getPropertySafelyAsString("JNP").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Wilayah").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Vehicle_Category").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Exco").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Rate_Bawah").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Rate_Tengah").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Rate_Atas").toString()),
                            Utility.isAnytype(tableRow.getPropertySafelyAsString("Jaminan").toString()));
                }
