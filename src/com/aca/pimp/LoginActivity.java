...
import com.aca.pimp.GetUserDataWS.GetuserdataListener;
import com.aca.pimp.ResetPasswordWS.ResetPassword;
			...
			try{
	        	...
	        	{   
		            ...
		            if (!error)
		            {
			            ...
			            {
		            		File fileOrDirectory = new File(Environment.getExternalStorageDirectory() + "/LoadImgPimp/");
			            ...
			            ***/
			            ...
			            dba.insert(
		            				...
		            				, Utility.isAnytype(tableRow.getPropertySafelyAsString("USER_NAME").toString())
		            				, tableRow.getPropertySafelyAsString("U_NAME").toString()
		            				, Utility.md5(tableRow.getPropertySafelyAsString("U_PASS").toString())
		            				, tableRow.getPropertySafelyAsString("ID_LEADER").toString()
		            				, tableRow.getPropertySafelyAsString("ID_LEVEL").toString()
		            				, tableRow.getPropertySafelyAsString("IS_ACHIEVED").toString()
		            				, tableRow.getPropertySafelyAsString("STATUS_LEVEL").toString()
		            				, tableRow.getPropertySafelyAsString("ID_ROLE").toString()
		            				, tableRow.getPropertySafelyAsString("LIMIT_ASRI").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_OTOMATE").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_MOTORCAR").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_TS_DOM").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_TS_INT").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_MEDISAFE").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_WELLWOMAN").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_DNO").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_MARINE_CARGO").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_OTOMATE_SYARIAH").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_ASRI_SYARIAH").toString()
								    , tableRow.getPropertySafelyAsString("LIMIT_PA_AMANAH").toString()
...
//					ShowDialog(R.layout.dialog_remove_userid, "Konfirmasi", R.id.dlgOK, R.id.dlgCancel);
					...
					if(error){
						...
						SharedPreferences sharedPreferences = LoginActivity.this.getSharedPreferences("com.aca.pimp", Context.MODE_PRIVATE);
						sharedPreferences
							.edit()
