...
import com.aca.database.DBA_MASTER_MARINE_RATE;
...
import com.aca.pimp.GetExchangeRate.GetexchangerateInterface;
...
ArrayList<HashMap<String, String>> custList ;
	...
	private RadioButton rbRateAtas,
						rbRateTengah,
						rbRateBawah;
...
//			MAX_DISCOUNT = Utility.getMaxDiscount(getBaseContext(), "11");
	...
	private void getIntentData() {
		...
		if (getIntent().getExtras() != null) {
			...
			LoadPimpMarine ws = new LoadPimpMarine(this, hmap) {

				@Override
				public void getMarineList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
						try {
							...
							vesselType.setSelection(a.getItemId(hMap.get("CONVEYANCE_TYPE"))); 
							vesselName.setText(hMap.get("CONVEYANCE_NAME"));  
							vesselweight.setText(hMap.get("VESSEL_WEIGHT"));
							vesselyear.setText(hMap.get("VESSEL_YEAR"));
							
							voyageno.setText(hMap.get("VOYAGE_NO"));
							
							etd.setText(hMap.get("EFF_DATE"));
							
							blno.setText(hMap.get("BL_NO"));
							lcno.setText(hMap.get("LC_NO"));
							voyagefrom.setText(hMap.get("VOYAGE_FROM"));
							voyageto.setText(hMap.get("VOYAGE_TO"));
							transhipment.setText(hMap.get("VOYAGE_TRANS"));

							a = (SpinnerGenericAdapter)spPolicyType.getAdapter();
							spPolicyType.setSelection(a.getItemId(hMap.get("POLICY_TYPE")));
							
							
							a = (SpinnerGenericAdapter)spCurrency.getAdapter();
							spCurrency.setSelection(a.getItemId(hMap.get("CURRENCY")));

							exchangeRate.setText(nf.format(Double.parseDouble(hMap.get("EXCHANGE_RATE"))));	
							si.setText(nf.format(Double.parseDouble(hMap.get("TOTAL_SI")))); 
							exchangePremi.setText(nf.format(Double.parseDouble(hMap.get("TOTAL_PREMIUM")) * Double.parseDouble(hMap.get("EXCHANGE_RATE"))));
					 
							int jenisRate = Integer.parseInt(hMap.get("TIPE_RATE"));
							if (jenisRate == 0)
							...
							total.setText(nf.format(Double.parseDouble(hMap.get("TOTAL_PREMIUM")) * Double.parseDouble(hMap.get("EXCHANGE_RATE"))));
			...
			LoadPimpMarineText wsMarine = new LoadPimpMarineText(FillCargoActivity.this, hmap) {
				
				@Override
				protected void getMarineList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
						String intDetail = "";
						for (HashMap<String, String> hashMap : arrList) {
							intDetail += hashMap.get("DESCRIPTION");
						}
						interestdetails.setText(intDetail); 	
...
//			vesselName.setTag(R.string.vessel_name, c.getString(8));
			...
			if (c.getString(31).equals("0"))
				rbRateBawah.setChecked(true);
			else if (c.getString(31).equals("1"))
				rbRateTengah.setChecked(true);
			else 
				rbRateAtas.setChecked(true);
...
//		vesselType.setTextColor(Color.BLACK);
	...
	private void RegisterListener(){
		rgRate.setOnCheckedChangeListener(new OnCheckedChangeListener() {
			
			@Override
			public void onCheckedChanged(RadioGroup group, int checkedId) { 
				DBA_MASTER_MARINE_RATE dbMarine = new DBA_MASTER_MARINE_RATE(FillCargoActivity.this);
				Cursor c = null;
				...
				try {
					...
					rateBawah = c.getDouble(0);
					rateTengah = c.getDouble(1);
					rateAtas = c.getDouble(2);
					...
					FillCargoActivity.this.rate.setText(rate + "");
...
//		}
		...
		if( si.getText().toString().isEmpty() || nf.parse(si.getText().toString()).doubleValue() == minTSI ){
		...
		if (!rbRateAtas.isChecked()   &&
			!rbRateTengah.isChecked() &&
			!rbRateBawah.isChecked() ) {
			flag ++ ;
			UIErrorMessage = "Jenis rate harus dipilih";
			return;
		}
		
		if(nf.parse(exchangePremi.getText().toString()).doubleValue() < minPremi) {
			flag++;
			UIErrorMessage = "Min Premi " + nf.format(minPremi) + " rupiah";
			return;
		}
	...
	public void btnNextClick(View v) throws ParseException
	{
		if (PRODUCT_ACTION.equals("APPROVE")) {
			...
			b.putDouble("TSI", nf.parse(si.getText().toString()).doubleValue());
			i.putExtras(b);
			Utility.setTempactivity((Activity)FillCargoActivity.this);
			startActivity(i);
		}
		else { 
			try{
				validasiNext();
				if(flag != 0)
					Utility.showCustomDialogInformation(FillCargoActivity.this, "Warning", UIErrorMessage);
...
//							((SpinnerGenericItem)spCurrency.getSelectedItem()).getCode(), 
							...
							"CARGO", 
							customername.getText().toString(),
							rbRateBawah.isChecked() ? "0" : rbRateTengah.isChecked() ? "1" : "2"
							);
				}
				else {
					dba2.nextInsert(
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillCargoActivity.this, "Informasi", "Produksi gagal di-approve").show();
			}
		};
		ws.execute(); 
	}
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillCargoActivity.this, "Informasi", "Produksi gagal di-reject").show();
