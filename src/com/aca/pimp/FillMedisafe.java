...
//			MAX_DISCOUNT = Utility.getMaxDiscount(getBaseContext(), "31");
	...
	private void getIntentData() {
		...
		if (getIntent().getExtras() != null) {
			...
			LoadPimpMedisafe ws = new LoadPimpMedisafe(this, hmap) {

				@Override
				public void getMedisafeList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
						...
						try {
							...
							if(hMap.get("Q2_FLAG").equals("1"))
								penyakit.setChecked(true);
							
							perawatandesc.setText(hMap.get("Q1_NOTE"));
							penyakitdesc.setText(hMap.get("Q2_NOTE"));							
							norek.setText(hMap.get("NCB_ACCOUNT_NO"));
							bank.setText(hMap.get("NCB_ACCOUNT_BANK"));
							namapemegang.setText(hMap.get("NCB_ACCOUNT_NAME"));
							...
							else if(hMap.get("PRODUCT_PLAN_FLAG").equals("3"))
							{
								plan3.setChecked(true);
							}
							else if(hMap.get("PRODUCT_PLAN_FLAG").equals("4"))
							{
			...
			LoadPimpFamily wsfam = new LoadPimpFamily(this, hmap) {
				...
				protected void getFamilyList(ArrayList<HashMap<String, String>> arrList) {
					if (arrList != null) {
						...
						fillFam(arrList.get(1), namapasangan, etPasangan, noktppasangan, pasangan);
						fillFam(arrList.get(2), namaanak1, etAnak1, noktpanak1, anak1);
						fillFam(arrList.get(3), namaanak2, etAnak2, noktpanak2, anak2);
						fillFam(arrList.get(4), namaanak3, etAnak3, noktpanak3, anak3); 
					}						
				}
...
//				}
	...
	public void btnNextClick(View v) {
		if (PRODUCT_ACTION.equals("APPROVE")) {
			...
			b.putDouble("TSI", 0.0);
			i.putExtras(b);
			Utility.setTempactivity((Activity)FillMedisafe.this);
			startActivity(i);
		}
		else { 
			try{
...
//		}
	...
	private void getPremi(){
		...
		if (umur>60) {
			findViewById(R.id.btnNext).setVisibility(View.GONE);
			Utility.showCustomDialogInformation(this, "Informasi", "Maksimal umur 60 tahun");
			return;
		}
		if (umur<18) {
			findViewById(R.id.btnNext).setVisibility(View.GONE);
			Utility.showCustomDialogInformation(this, "Informasi", "Minimal umur 18 tahun");
			return;
		}
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillMedisafe.this, "Informasi", "Produksi gagal di-approve").show();
			}
		};
		ws.execute(); 
	}
		...
		ApproveProduction ws = new ApproveProduction(this, hmap) { 
			...
			protected void getFlagApprovalProduction(Boolean flag) {
				if (flag) 
					...
					Utility.showCustomDialogInformation(FillMedisafe.this, "Informasi", "Produksi gagal di-reject").show();
