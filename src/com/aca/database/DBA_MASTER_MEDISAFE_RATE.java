...
public class DBA_MASTER_MEDISAFE_RATE {
    ...
    private static final String TAG = "DBA_MASTER_MEDISAFE_RATE";
    
    private static final String DATABASE_NAME = "AMM_VERSION_BIG";
    private static final String DATABASE_TABLE = "MASTER_MEDISAFE_RATE";

     static final String DATABASE_CREATE =
        "CREATE TABLE MASTER_MEDISAFE_RATE (TYPE NUMERIC, PLAN NUMERIC, AGE NUMERIC, PREMI NUMERIC);";
    ...
    public DBA_MASTER_MEDISAFE_RATE(Context ctx) 
    {
        this.context = ctx;
    ...
    //---opens the database---
    public DBA_MASTER_MEDISAFE_RATE open() throws SQLException 
    {
    ...
    public Cursor getRate(String type, String plan, String age) 
    {
    	if (plan == "4")
    	{
	    	...
	    	return db.rawQuery("SELECT TYPE, PLAN, AGE, PREMI FROM MASTER_MEDISAFE_RATE" +
	    			" WHERE TYPE = ? AND" +
	    			"		PLAN = ? AND " +
	    			"		AGE  = ? " +
	    			"	ORDER BY 2,3 " +
